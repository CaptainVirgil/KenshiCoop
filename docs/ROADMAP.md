...
| # | Item |
|---|---|
| 56 | **Weather is not synced, and cannot be at this deps pin.** Confirmed live in the same biome at close proximity, so it is a real divergence, not Kenshi's per-biome behaviour: each client rolls its own weather RNG per region and there is no seed field to replicate, so the state itself would have to be. **Blocked on the engine surface, not on design.** `kenshi/Weather.h` has zero `_NV_` wrappers, so `GetRealAddress` cannot resolve `WeatherSystem::getInstance()`; KenshiLib exports eight data globals and none is the weather system; nothing reachable holds a pointer to it; and raw RVA + image base does not resolve (measured: `0x73E920` lands in int3 padding). Re-check on the next deps bump, or ask upstream to wrap the Weather classes — the dump already has the full layout. `CharacterHuman::_NV_weatherUpdate` IS hookable if the goal ever narrows to equalising acid-rain damage rather than appearance |
