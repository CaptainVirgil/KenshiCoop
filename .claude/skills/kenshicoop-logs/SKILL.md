...
**The filename lies.** It reflects the panel Role, not the negotiated one — a
`KenshiCoop_host.log` is often the join's log. Establish the role first:
...
| `dormPc` | dormant bodies within the attention radius of a player character — **must be 0** |
