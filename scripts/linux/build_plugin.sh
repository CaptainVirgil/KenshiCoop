...
# Stamp WHICH KenshiLib this DLL was compiled against, so the plugin can log it
# next to the one actually serving it at runtime. Those are different things -
# the deps are pinned at v0.1 while RE_Kenshi ships 0.4.0 - and until this existed
# nothing recorded the pair, so "what were they running?" was unanswerable after a
# crash report.
#
...
printf '// Generated by build_plugin.sh - do not edit, do not commit.\n#define KENSHICOOP_DEPS_PIN "%s"\n' \
    "$DEPS_PIN" > "$REPO/build/generated/DepsPin.h"
