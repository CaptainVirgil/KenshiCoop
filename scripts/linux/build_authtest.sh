#!/usr/bin/env bash
# Builds and RUNS dist/authtest.exe with the same v100 toolchain, through Wine.
...
# links them against a FAKE coop::engine (src/authtest/EngineFakes.cpp) and
# the three cross-TU Replicator member stubs (src/authtest/ReplicatorStubs.cpp)
...
mkdir -p "$REPO/dist" "$REPO/build/authtest"
rm -f "$REPO/dist/authtest.exe"

echo "=== Building authtest.exe (Release|x64, v100 via Wine) ==="
...
nice -n 19 "$WINE_BIN/wine" "$VC/bin/amd64/cl.exe" \
    ...
    /Fo"$(winpath "$REPO/build/authtest/")" \
    /Fe"$(winpath "$REPO/dist/authtest.exe")" \
    "$(winpath "$REPO/src/authtest/main.cpp")" \
    "$(winpath "$REPO/src/authtest/EngineFakesAuth.cpp")" \
    "$(winpath "$REPO/src/authtest/NetLinkFakes.cpp")" \
    "$(winpath "$REPO/src/drivetest/EngineFakes.cpp")" \
    "$(winpath "$REPO/src/authtest/AuthStubs.cpp")" \
    "$(winpath "$REPO/src/plugin/sync/ReplicatorAuthority.cpp")" \
    "$(winpath "$REPO/src/plugin/sync/ReplicatorPublish.cpp")" \
...
[ -f "$REPO/dist/authtest.exe" ] || { echo "authtest build FAILED" >&2; exit 1; }
echo "authtest built: $REPO/dist/authtest.exe"

echo "=== Running authtest (Wine, ~20 s of real-time pacing) ==="
# Run from build/authtest so authtest.log lands there, not in the repo root.
cd "$REPO/build/authtest"
set +e
nice -n 19 "$WINE_BIN/wine" "$(winpath "$REPO/dist/authtest.exe")"
rc=$?
echo "authtest exit code: $rc (log: $REPO/build/authtest/authtest.log)"
exit $rc
